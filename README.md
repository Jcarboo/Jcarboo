![Typing SVG](https://readme-typing-svg.demolab.com?font=Press+Start+2P&size=18&pause=1200&color=F7D000&center=true&vCenter=true&width=700&lines=Jayson+Carboo;ML+%2B+Full-Stack;Software+Dev)

<img src="https://i.pinimg.com/originals/c6/33/c2/c633c20ede82f0e0ced7d570dbe3a1f3.gif" width="100%" />

<br><br>

![GitHub Streak](https://streak-stats.demolab.com?user=jcarboo&theme=tokyonight)

<br>

</div>

<!-- ABOUT ME (GAME UI) -->
<div align="center">
<img src="https://i.pinimg.com/originals/05/bf/b1/05bfb13dbad7c8f3815c723d4c0c34be.gif" width="10%" />
<h2> About Me</h2>

<table>
<tr>
    <td valign="top" style="padding:18px; border:1px solid #30363d; border-radius:14px; background:#0d1117; min-width:320px;">
<div class="abilities-box">      
<b>NAME:</b> Jayson Carboo<br/>
<b>CLASS:</b> ML + Full-Stack + Software Developer<br/>
<b>LOCATION:</b> USA<br/>

<b> Current Quest</b><br/>
Sepsis prediction app using high level machine learning on patient data<br/><br/>

<b> Bio </b><br/>
Hi! I'm developer with a passion for Full-Stack developlment and software development.<br>
Check out my repos to see some stuff I've built

<b> Active Projects </b><br/>
• Dreamer — AI-powered study planner<br/>
• Farm-Profit-Tool — Calculate net return based on farm costs using ML<br/>
</div>

</td>
</tr>
</table>

</div>

<br/>

<!-- SKILLS (BOXES) -->
<div align="center">

<h2 class="block-craft-h3"> Skill Set</h2>

<table>
<tr>
  <!-- Frontend -->
  <td valign="top" style="padding:18px; border:1px solid #30363d; border-radius:14px; background:#0d1117;">
    <h3 align="left" style="margin:0 0 12px 0;">Frontend</h3>
    <div>
      <img src="https://skillicons.dev/icons?i=html" height="48" />
      <img src="https://skillicons.dev/icons?i=css" height="48" />
      <img src="https://skillicons.dev/icons?i=bootstrap" height="48" />
      <img src="https://skillicons.dev/icons?i=tailwind" height="48" />
      <br />
      <img src="https://skillicons.dev/icons?i=js" height="48" />
      <img src="https://skillicons.dev/icons?i=react" height="48" />
      <img src="https://skillicons.dev/icons?i=nextjs" height="48" />
    </div>
  </td>

  <td style="width:18px;"></td>

  <!-- Languages -->
  <td valign="top" style="padding:18px; border:1px solid #30363d; border-radius:14px; background:#0d1117;">
    <h3 align="left" style="margin:0 0 12px 0;">Languages</h3>
    <div>
      <img src="https://skillicons.dev/icons?i=c" height="48" />
      <img src="https://skillicons.dev/icons?i=cpp" height="48" />
      <img src="https://skillicons.dev/icons?i=java" height="48" />
      <img src="https://skillicons.dev/icons?i=py" height="48" />
      <br />
      <img src="https://skillicons.dev/icons?i=js" height="48" />
      <img src="https://skillicons.dev/icons?i=ts" height="48" />
      <img src="https://skillicons.dev/icons?i=postgres" height="48" />
    </div>
  </td>

  <td style="width:18px;"></td>

  <!-- Others -->
  <td valign="top" style="padding:18px; border:1px solid #30363d; border-radius:14px; background:#0d1117;">
    <h3 align="left" style="margin:0 0 12px 0;">Others</h3>
    <div>
      <img src="https://skillicons.dev/icons?i=git" height="48" />
      <img src="https://skillicons.dev/icons?i=github" height="48" />
      <img src="https://skillicons.dev/icons?i=npm" height="48" />
      <img src="https://skillicons.dev/icons?i=figma" height="48" />
      <br />
      <img src="https://skillicons.dev/icons?i=vscode" height="48" />
      <img src="https://skillicons.dev/icons?i=vercel" height="48" />
      <img src="https://skillicons.dev/icons?i=linux" height="48" />
      <img src="https://skillicons.dev/icons?i=sklearn" height="48" />
    </div>
  </td>
</tr>
</table>

</div>


<br/>


<br/>

<!-- LINKS -->
<div align="center">

<h2>🔗 Connect</h2>

<table>
  <tr>
    <td valign="top" style="padding:18px; border:1px solid #30363d; border-radius:14px; background:#0d1117; min-width:320px;">
      
<b> Portfolio:</b> https://jcarboo.vercel.app<br/>
<b> LinkedIn:</b> https://www.linkedin.com/in/jayson-carboo/<br/>
<b> Email:</b> carbooj22@gmail.com<br/><br/>
  </tr>
</table>

</div>
